...
export default function Edit() {
	...
	const onSubmitForm = async (e) => {
		...
		try {
			...
			navigate("/profile", {state: {id: response.data.id, userName: response.data.userName, email: response.data.email, phoneNumber: response.data.phoneNumber, role: response.data.role}});
		}
		catch (err)
		{
			alert("Update Unsuccessful");
			const res = axios.get(`http://localhost:8080/user/${id}`);
